...
function addGamesToPage(games) {
  ...
  for (var game of games) {
    ...
    const displayCard = `
      <div>
          <h2 style="color: #000;">${game.name}</h2>
          <p style="color: #000;">${game.description}</p>
...
const totalContributions = GAMES_JSON.reduce((acc, game) => acc + game.backers, 0); // (accumulator, currValue)
...
const totalRaised = GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0); // (accumulator, currValue)
...
let sortedGames = GAMES_JSON.sort((a, b) => b.pledged - a.pledged);
...
// Creare a search bar to filter games by name
function searchGames() {
  ...
  // Filter games where the name contains any of the search words as a whole word
  const filteredGames = GAMES_JSON.filter((game) => {
    const gameName = game.name.toLowerCase();
    return searchWords.some((word) => gameName.includes(word));
  });
  ...
  const existingResults = descriptionContainer.querySelectorAll(
    "p:not(:first-child)"
  );
  existingResults.forEach((result) => result.remove());

  // Add the new search results
  const displayStr = `Search results for "${searchInput}": ${filteredGames.length} game(s) found.`;
  const searchResult = document.createElement("p");
  searchResult.textContent = displayStr;
  descriptionContainer.appendChild(searchResult);
}
